...
static cairo_surface_t *screen_sdl_surface_create(screen_sdl_priv_t *priv) {
    ...
    priv->draw_surface = SDL_CreateRGBSurfaceWithFormat(0, 128, 64, 8, SDL_PIXELFORMAT_INDEX8);
    surface = cairo_image_surface_create_for_data((unsigned char *)priv->draw_surface->pixels,
                                                  CAIRO_FORMAT_A8, priv->draw_surface->w, priv->draw_surface->h,
                                                  cairo_format_stride_for_width(CAIRO_FORMAT_A8, priv->draw_surface->w));
